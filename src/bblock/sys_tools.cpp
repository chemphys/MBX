...
void SetCharges(std::vector<double> xyz, std::vector<double> &charges, std::string mon_id, size_t n_mon, size_t nsites,
    ...
    // Alkali metal ions
        ...
        for (size_t nv = 0; nv < n_mon; nv++) {
            charges[fst_ind + nv * nsites + 0] = 0.392265 * CHARGECON;
            charges[fst_ind + nv * nsites + 1] = -0.59806625 * CHARGECON;
            charges[fst_ind + nv * nsites + 2] = -0.59806625 * CHARGECON;
            charges[fst_ind + nv * nsites + 3] = -0.59806625 * CHARGECON;
            charges[fst_ind + nv * nsites + 4] = -0.59806625 * CHARGECON;
        }

    } else if (mon_id == "co3a") {
        for (size_t nv = 0; nv < n_mon; nv++) {
            charges[fst_ind + nv * nsites + 0] = 0.0565 * CHARGECON;
            charges[fst_ind + nv * nsites + 1] = -0.6855 * CHARGECON;
...
void SetPolfac(std::vector<double> &polfac, std::string mon_id, size_t n_mon, size_t nsites, size_t fst_ind,
    ...
    if (mon_id == "li") {  // Lithium
        ...
        for (size_t nv = 0; nv < n_mon; nv++) {
            polfac[fst_ind + nv * nsites + 0] = 4.0761;
            polfac[fst_ind + nv * nsites + 1] = 1.1228;
            polfac[fst_ind + nv * nsites + 2] = 1.1228;
            polfac[fst_ind + nv * nsites + 3] = 1.1228;
            polfac[fst_ind + nv * nsites + 4] = 1.1228;
        }

    } else if (mon_id == "co3a") {
        for (size_t nv = 0; nv < n_mon; nv++) {
            polfac[fst_ind + nv * nsites + 0] = 2.8749;
            polfac[fst_ind + nv * nsites + 1] = 1.4785;
...
void SetPol(std::vector<double> &pol, std::string mon_id, size_t n_mon, size_t nsites, size_t fst_ind,
    ...
    if (mon_id == "li") {  // Lithium
        ...
        for (size_t nv = 0; nv < n_mon; nv++) {
            pol[fst_ind + nv * nsites + 0] = 4.0761;
            pol[fst_ind + nv * nsites + 1] = 1.1228;
            pol[fst_ind + nv * nsites + 2] = 1.1228;
            pol[fst_ind + nv * nsites + 3] = 1.1228;
            pol[fst_ind + nv * nsites + 4] = 1.1228;
        }

    } else if (mon_id == "co3a") {
        for (size_t nv = 0; nv < n_mon; nv++) {
            pol[fst_ind + nv * nsites + 0] = 2.8749;
...
void SetC6LongRange(std::vector<double> &c6_lr, std::string mon_id, size_t n_mon, size_t natoms, size_t fst_ind,
    ...
    if (mon_id == "f") {  // Fluoride
        ...
        for (size_t nv = 0; nv < n_mon; nv++) {
            c6_lr[nv * natoms + fst_ind] = 41.30693525305406;   // A
            c6_lr[nv * natoms + fst_ind] = 18.604244139443022;  // B
            c6_lr[nv * natoms + fst_ind] = 18.604244139443022;  // B
            c6_lr[nv * natoms + fst_ind] = 18.604244139443022;  // B
            c6_lr[nv * natoms + fst_ind] = 18.604244139443022;  // B
        }
        ...
        for (size_t nv = 0; nv < n_mon; nv++) {
            c6_lr[nv * natoms + fst_ind] = 29.870569127487343;  // A
